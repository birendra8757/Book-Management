...
const updateBooks = async function (req, res) {
  try {
    ...
    const isIsbnAlreadyUsed = await bookModel.findOne({
      ISBN,isDeleted: false,});
    const currentDate = moment().format('MMMM Do YYYY, h:mm:ss a')
    req.body["releasedAt"] = currentDate
    const update = await bookModel.findOneAndUpdate(
      { _id: bookId, isDeleted: false },
      { $set: {title: title, excerpt: excerpt, currentDate: releasedAt,ISBN: ISBN,}, },{ new: true } );
    if (!update) {return res.status(404).send({ status: false, message: "Book is not exist" }); }
      else{
    return res .status(200).send({ status: true, message: "Book List", data: update });}
  } catch (error) {
    return res.status(500).send({ status: false, message: error.message });}}
...
const DeletedBook = async function (req, res) {
  try {
    ...
    if (!savedata) { return res.status(404).send({ status: false, message: "book not found for this id so can't delete anything" }) }
    if (savedata.isDeleted == true) {
      return res.status(404).send({ status: false, message: "book is already deleted" })}
    const deleteBook = await bookModel.findByIdAndUpdate({ _id: bookId }, { $set: { isDeleted: true, deletedAt: new Date() } });
